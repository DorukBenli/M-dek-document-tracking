...
class DocumentController
{
    ...
    public function add($documentTypes)
    {
        foreach ($documentTypes as $type) {
            ...
            $result = $this->documentModel->createDocument($type, $exam, $soft, $type);
            if (!$result) {
    ...
    public function addOther($documentTypes, $name)
    {
        foreach ($documentTypes as $type) {
            $exam = 0;
            $soft = 0;
            ...
            // Call the insert function with the appropriate parameters
            $result = $this->documentModel->createDocument($type, $exam, $soft, $name);
            if (!$result) {
                // Handle the failure as needed
                return false;
            }
    ...
    public function addRequiredDocuments($documentTypes)
    {
        foreach ($documentTypes as $document) {
            ...
            if (strpos($document, '- Best') !== false || strpos($document, '- Average') !== false || strpos($document, '- Worst') !== false) {
                $requirementType = 'MUDEK';
            } else {
                $requirementType = 'YÖK';
            }
