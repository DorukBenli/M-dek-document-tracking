...
class UserModel
{
    ...
    public function getAllSofts($term, $crn)
    {
        // Prepare the SQL query to retrieve all document types for the given course and term
        $stmt = $this->conn->prepare("SELECT document_type FROM bitirme.soft_submit WHERE term = ? AND crn = ?");
        ...
        // Initialize an empty array to store document types
        $documentTypes = [];

        // Fetch each row and store the document type in the array
        while ($row = $result->fetch_assoc()) {
        ...
        // Return the array of document types
