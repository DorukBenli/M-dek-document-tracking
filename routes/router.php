...
$softsubmitController = new SoftSubmitController($conn);
...
switch ($action) {
    ...
    case 'removeExam':
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['term']) && isset($_POST['course_code']) && isset($_POST['section_code']) && isset($_POST['document_type']) && isset($_POST['crn'])) {
            ...
            $result = $softsubmitController->removeExamFromCourse($term, $crn, $document);
    ...
    case 'addExam':
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['term']) && isset($_POST['crn']) && isset($_POST['exam'])) {
            ...
            $isOther=false;
            // Check if the $exam has 'Other'
            if (strpos($exam, 'Other') !== false && isset($_POST['other_exam'])) {
                $isOther=true;
                $name=$_POST['other_exam'];
            }
            // Check if the document exists
            $documentExists = $documentController->getDocument($exam);

            if (!$documentExists) {
                if (substr($exam, 0, 7) === 'Midterm' || substr($exam, 0, 5) === 'Final') {
                    ...
                    $documentTypes = array($exam);
                }
                $documentController->add($documentTypes);
                $documentController->addRequiredDocuments($documentTypes);
            }
            if($isOther) {
                $result = $submitController->addOther($term, $crn, $exam, $name);
                $result = $softsubmitController->addOther($term, $crn, $exam, $name);
            }
            else {
                $result = $submitController->addExam($term, $crn, $exam);
                $result = $softsubmitController->add($term, $crn, $exam);
            }
            if ($result) {
                $userController->showCoursePage($crn, $term);
                exit; // Stop script execution after redirecting
    ...
    case 'addMakeup':
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['term']) && isset($_POST['crn']) && isset($_POST['document_type'])) {
            $term = $_POST['term'];
            $crn = $_POST['crn'];
            ...
            if (!isSubmit($term, $crn, $exam)) {
                $result = $submitController->addMakeup($term, $crn, $exam);
            }

            $result = $softsubmitController->addMakeup($term, $crn, $exam);
    ...
    case 'submitFileArg':
        if (!isset($_POST['documents_submit'])) {
            $_POST['documents_submit'] = [];
        }

        // Check if documents_undo key is not set and set it to an empty array
        if (!isset($_POST['documents_undo'])) {
            $_POST['documents_undo'] = [];
        }
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['term']) && isset($_POST['crn']) && isset($_POST['documents_submit']) && isset($_POST['documents_undo'])) {
            $term = $_POST['term'];
            $crn = $_POST['crn'];
            $submittedDocuments = $_POST['documents_submit'];
            $undoDocuments = $_POST['documents_undo'];

            $result = false;

            foreach ($submittedDocuments as $document) {
                // Do something with each submitted document
                $document = str_replace('_', ' ', $document);
                $result = $softsubmitController->updateDocumentArg($term, $crn, $document);
            }

            foreach ($undoDocuments as $document) {
                // Do something with each submitted document
                $document = str_replace('_', ' ', $document);
                $result = $softsubmitController->undoDocumentArg($term, $crn, $document);
            }
