...
$handles_relationships = getHandlesRelationship($username);
...
foreach ($handles_relationships as $handles_relationship) {
    // Get the course code from the relationship
    $crn = $handles_relationship['crn'];
...
$filteredCourses = array_filter($courses_handled, function ($course) use ($searchTerm) {
    ...
    <main>
        ...
        <?php if ($total_courses == 0 && isset($_GET['search']) === false) : ?>
            <div class="no-courses-message">
                You didn't handle any courses in <?php echo $selectedTerm; ?> semester.
            </div>
        ...
        <?php else : ?>
            ...
            <?php foreach ($paginated_courses as $index => $course) : ?>
                ...
                <?php if (end($courses_handled)['crn'] != $course['crn'] || $courses_handled[0]['crn'] == end($courses_handled)['crn'] || $index == 1) : ?>
